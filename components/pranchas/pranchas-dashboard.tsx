          ...
          {active && (
            <>
              <Button
                ...
                onClick={() => edit(active)}
              >
function FleetOperationsDrawer({
  fleet,
  fretes,
  manutencoes,
  close,
  ...
  const related = fleet
    ? fretes.filter(
        (frete) =>
          frete.frota === fleet.numero ||
          getEquipeTransporte(frete).some(
            (team) => team.frota === fleet.numero
          )
      )
    : [];

  const active = related.find(
    (frete) => frete.status === "Em Frete"
  );

  const last = active || related[0];

  const activeMaintenance = fleet
    ? manutencoes.find(
        (m) =>
          m.frota === fleet.numero &&
          m.status === "Em manutenção"
  ...
  const driver =
    active && fleet
      ? getEquipeTransporte(active).find(
          (team) => team.frota === fleet.numero
        )?.motorista || active.motorista
      : undefined;
    ...
    >
      {fleet && (
        <div className="p-5">
          <div className="flex items-start justify-between gap-3 border-b border-[var(--ds-border)] pb-4">
            <div>
              <p className="ds-caption uppercase tracking-[.1em]">
                Prancha
              </p>
            ...
            {activeMaintenance ? (
              <>
                <Button
                  variant="secondary"
                  onClick={() =>
                    editMaintenance(activeMaintenance)
                  }
                >
                  <Pencil size={14} />
                  Editar manutenção
                ...
                <Button
                  variant="primary"
                  onClick={() =>
                    finishMaintenance(activeMaintenance)
                  }
                >
                  <Check size={14} />
                  Finalizar manutenção
